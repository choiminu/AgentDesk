...
const usage = `agentdesk-recall <command>
  ...
  install                                          register hooks in ~/.claude/settings.json (+ gemini/codex if present) and the /recall skill
...
async function install() {
  ...
  const targets = [
    { agent: "claude", file: join(homedir(), ".claude", "settings.json"), end: "SessionEnd", prompt: "UserPromptSubmit" },
    { agent: "gemini", file: join(homedir(), ".gemini", "settings.json"), end: "SessionEnd", prompt: "BeforeAgent" },
    { agent: "codex", file: join(homedir(), ".codex", "hooks.json"), end: "SessionEnd", prompt: "UserPromptSubmit" },
  ];
  for (const t of targets) {
    ...
    s.hooks = s.hooks || {};
    for (const [ev, script] of [[t.end, "on-session-end.mjs"], [t.prompt, "on-prompt.mjs"]]) {
